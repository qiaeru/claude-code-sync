...
- **Path-traversal rejection.** Archive entries that try to escape the target folders (`..` segments, absolute paths, drive letters) are rejected, so a malicious or corrupted archive cannot write outside the chosen directories. A destination that resolves outside its target through a symlink (e.g. a dotfile-managed `~/.claude/settings.json`) is not written through either, and is reported as skipped in the plan rather than silently dropped.
- **Integrity verification.** Each entry carries a SHA-256 in `manifest.json`. All checksums are verified against the extracted files *before anything is written*, and a destination that cannot be written as a file (it is an existing folder, or sits under an existing file) is flagged as skipped in the plan instead of failing midway, so a corrupted archive aborts the restore without leaving a half-restored tree behind. A restorable entry with no recorded checksum aborts the import too (every export records one, so its absence means a tampered or hand-crafted manifest). The same checksum pass can be run on demand, without restoring, via `claude-code-sync verify`. (The hash guards against accidental corruption; the authenticated WinZip-AES encryption already blocks tampering by anyone *without* the password.)
...
- Binds to `127.0.0.1` only and is never exposed to the network; the `--host` flag refuses non-loopback addresses at startup, so the server cannot be bound to a LAN interface by accident.
