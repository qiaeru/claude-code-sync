...
- Rejects cross-site and DNS-rebinding requests: the `Host` header must be present and local and the `Origin` (when sent) must be local, which blocks other websites or processes from driving the API.
